...
def extract_feature_test(path):
    ...
    # mel_var = pd.Series()
    for i in range(1, 21):
        exec(f'mfcc{i}_mean = pd.Series()')
        exec(f'mfcc{i}_var = pd.Series()')
    mfcc_mean = pd.Series()
    mfcc_std = pd.Series()
    mfcc_var = pd.Series()
    ...
    for line in file_data:
        ...
        songname_vector.at[id] =  line  # song name ############################ modified name
        ...
        ########### MFCC LABELS #################
        for i in range(1, 21):
            exec(f'mfcc{i}_mean.at[id] = np.mean(mfcc[i-1])')
            exec(f'mfcc{i}_var.at[id] = np.var(mfcc[i-1])')
    ...
    feature_set['rms_mean'] = rmse_mean  # rmse
    # feature_set['rms_std'] = rmse_std
    feature_set['rms_var'] = rmse_var
    feature_set['spectral_centroid_mean'] = cent_mean  # cent
    # feature_set['spectral_centroid_std'] = cent_std
    feature_set['spectral_centroid_var'] = cent_var
    feature_set['spectral_bandwidth_mean'] = spec_bw_mean  # spectral bandwidth
    # feature_set['spectral_bandwidth_std'] = spec_bw_std
    feature_set['spectral_bandwidth_var'] = spec_bw_var
    feature_set['rolloff_mean'] = rolloff_mean  # rolloff
    # feature_set['rolloff_std'] = rolloff_std
    feature_set['rolloff_var'] = rolloff_var
    feature_set['zero_crossing_rate_mean'] = zcr_mean  # zero crossing rate
    # feature_set['zcr_std'] = zcr_std
    feature_set['zero_crossing_rate_var'] = zcr_var
    feature_set['harmony_mean'] = harm_mean  # harmonic
    # feature_set['harm_std'] = harm_std
    feature_set['harmony_var'] = harm_var
    feature_set['perceptr_mean'] = perc_mean  # percussive
    # feature_set['perc_std'] = perc_std
    feature_set['perceptr_var'] = perc_var
    feature_set['tempo'] = tempo_vector  # tempo 
    for i in range(1, 21):
        exec(f"feature_set['mfcc{i}_mean'] = mfcc{i}_mean")
        exec(f"feature_set['mfcc{i}_var'] = mfcc{i}_var")
...
###################################
    ...
    # feature_set = feature_set.sort_values(by="song_name")

    # Converting Dataframe into CSV Excel and JSON file
    feature_set.to_csv('myelin_test_features_splitter.csv')
    #feature_set.to_json('Emotion_features_test.json')
